defmodule TicTacToeGameTest do
  ...
  test "games are created by joining" do
    assert {:ok, "X"} = TicTacToe.join_or_create_game(@game_name, %{token: "t", name: "rob"})
  end

  test "only 2 people can join a game" do
    assert {:ok, "X"} = TicTacToe.join_or_create_game(@game_name, %{token: "t", name: "rob"})
    assert {:ok, "O"} = TicTacToe.join_or_create_game(@game_name, %{token: "t2", name: "bob"})
    assert {:error, "game is full"} == TicTacToe.join_or_create_game(@game_name, %{token: "t3", name: "snob"})
  end

  test "the game state is broadcast when the game is ready" do
    @endpoint.subscribe(self(), @game_topic)
    assert {:ok, "X"} = TicTacToe.join_or_create_game(@game_name, %{token: "t1", name: "george"})
    assert {:ok, "O"} = TicTacToe.join_or_create_game(@game_name, %{token: "t2", name: "bob"})
  ...
  test "players can make a move" do
    ...
    assert {:ok, %{board: board, whose_turn: "O"}} = TicTacToe.move(@game_name, %{token: "t1", square: 0})
    assert board == ["X",nil,nil,nil,nil,nil,nil,nil,nil]
    assert {:ok, %{board: board, whose_turn: "X"}} = TicTacToe.move(@game_name, %{token: "t2", square: 1})
